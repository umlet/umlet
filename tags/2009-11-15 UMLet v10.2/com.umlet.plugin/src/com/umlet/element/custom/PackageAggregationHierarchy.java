...
public class PackageAggregationHierarchy extends com.umlet.element.base.Entity {
	...
	public void paintEntity(Graphics g) {
		...
		float yPos = 10 * zoom;
		float xPos = 10 * zoom;
		float packageHeight = this.getHandler().getZoomedFontsize();
		float packageWidth = 2 * this.getHandler().getZoomedFontsize();		
	...
	private void drawPackage(Graphics2D g2, float xPos, float yPos, float packageHeight, float packageWidth, String name) {
		...
		this.getHandler().writeText(g2, name, (int)(xPos + packageWidth + this.getHandler().getZoomedDistLineToText()+0.5), (int)(packageHeight + yPos+0.5), false);
	}
