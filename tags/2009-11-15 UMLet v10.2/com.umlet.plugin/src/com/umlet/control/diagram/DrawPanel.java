...
public class DrawPanel extends JPanel implements Printable {
	...
	public void warnIfAnyValueIsNotOnGrid() {
		...
		for (int i = 0; i < v.size(); i++) {
			...
			if (x != getHandler().ensureOnGrid(x)) JOptionPane.showMessageDialog(this, entity + " is not on Grid -> X: " + x);
			if (y != getHandler().ensureOnGrid(y)) JOptionPane.showMessageDialog(this, entity + " is not on Grid -> Y: " + y);
			if (w != getHandler().ensureOnGrid(w)) JOptionPane.showMessageDialog(this, entity + " is not on Grid -> W: " + w);
			if (h != getHandler().ensureOnGrid(h)) JOptionPane.showMessageDialog(this, entity + " is not on Grid -> H: " + h);

			if (entity instanceof Relation) {
				for (Point point : ((Relation) entity).getLinePoints()) {
					x = (int) point.getX();
					y = (int) point.getY();
					if (x != getHandler().ensureOnGrid(x)) JOptionPane.showMessageDialog(this, "Point of " + entity + " is not on Grid -> X: " + x);
					if (y != getHandler().ensureOnGrid(y)) JOptionPane.showMessageDialog(this, "Point of " + entity + " is not on Grid -> Y: " + y);
				}
			}
		}
	}
	...
	public void insertUpperLeftWhitespaceIfNeeded() {
		...
		if ((adjustWidth != 0) || (adjustHeight != 0)) {
			for (int i = 0; i < this.getComponents().length; i++) {
				Component c = this.getComponent(i);
				c.setLocation(handler.ensureOnGrid(c.getX() - adjustWidth), handler.ensureOnGrid(c.getY() - adjustHeight));
			}
		}
	...
	public void removeUnnecessaryWhitespaceAroundDiagram() {
		...
		for (int i = 0; i < this.getComponents().length; i++) {
			Component c = this.getComponent(i);
			c.setLocation(handler.ensureOnGrid(c.getX() - newX), handler.ensureOnGrid(c.getY() - newY));
		}
	...
	private void checkIfScrollbarsAreNecessary() {
		...
		int adx = 0;
		int ady = 0;
		if ((_scr.getHorizontalScrollBar().getValue() != 0) && vertWasVisible && !isVerticalScrollbarVisible()) adx = handler.ensureOnGrid(horSbHeight);
		if ((_scr.getVerticalScrollBar().getValue() != 0) && horWasVisible && !isHorizontalScrollbarVisible()) ady = handler.ensureOnGrid(verSbWidth);
