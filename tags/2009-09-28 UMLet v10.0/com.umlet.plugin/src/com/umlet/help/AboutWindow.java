...
public class AboutWindow extends HelpLabel {
	...
	public static AboutWindow getInstance() {
		if (window == null) window = new AboutWindow();
		return window;
	}
	...
	public void mouseEntered(MouseEvent e) {
		...
		if (this.getTopLevelAncestor() != null) this.getTopLevelAncestor().setCursor(Constants.handCursor);
	}

	@Override
	public void mouseMoved(MouseEvent e) {
		if (this.getTopLevelAncestor() != null) {
			if (link1.contains(e.getX(), e.getY()) || link2.contains(e.getX(), e.getY())) this.getTopLevelAncestor().setCursor(Constants.handCursor);
			else this.getTopLevelAncestor().setCursor(Constants.defCursor);
		}
	}
}
