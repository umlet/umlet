...
public class Container extends Element {
	...
	public Container(Graphics2D g, Container parent, ArrayList<Row> rows, int row) {
		super(g, 0, null);
		...
		if (Const.AutoInsertIF && (this.parent != null)) this.setStartElement(new If(g, null));
		else if (this.parent != null) this.setStartElement(new StartElement(g, 0, null));
	}

	public Container addNewContainer() {
		Container c = new Container(this.getGraphics(), this, this.rows, this.current_row);
		this.columns.get(this.columns.size() - 1).addElement(c);
		return c;
	}

	public Container addNewWhile(String condition) {
		Container c = new While(this.getGraphics(), this, this.rows, this.current_row, condition);
		this.columns.get(this.columns.size() - 1).addElement(c);
		return c;
	}
	...
	public Container close() {
		if (Const.AutoInsertIF && (this.parent != null)) this.setStopElement(new EndIf(this.getGraphics(), null));
		else if (this.parent != null) this.setStopElement(new StopElement(this.getGraphics(), 0, null));
		return this.parent;
	}
	...
	protected int getLeftWidth() {
		...
		width += ((this.columns.size() - 1)) / 2.0 * Const.COLUMN_PAD;
	...
	protected int getWidth() {
		...
		width += (this.columns.size() - 1) * Const.COLUMN_PAD;
	...
	public void setX(int x) {
		...
		if (this.columns.size() == 1) this.columns.get(0).setX(x);
			...
			for (Column c : this.columns) {
				...
				c.setX(x);
				x += c.getRightWidth();
				if (i < this.columns.size()) x += Const.COLUMN_PAD;
				i++;
			}
		}
	}
