...
public class ActivityDiagramText extends com.umlet.element.base.Entity {
	...
	private void init(Graphics2D graphics) {
		...
		zoom = (float) Umlet.getInstance().getDiagramHandler().getGridSize() / (float) Constants.defaultGridSize;
		...
		// Zoom Const values
		Const.PAD = (int) (10 * zoom);
		Const.COLUMN_PAD = (int) (30 * zoom);
		Const.GOTO_SEP = (int) (4 * zoom);
		Const.DIAGRAM_PAD = (int) (30 * zoom);
		Const.MIN_WIDTH = (int) (50 * zoom);
		Const.MIN_HEIGHT = (int) (50 * zoom);

		// Some unimportant initialization stuff; setting color, font
		// quality, etc. You should not have to change this.
		this.graphics = graphics;
		this.graphics.setFont(this.getHandler().getZoomedFont());
		this.graphics.setColor(_activeColor);
		this.getHandler().getFRC(this.graphics);
	}
	...
	public int getGotoPosition(Direction dir) {
		if (Direction.LEFT.equals(dir)) {
			if (this.goto_seperation_left + Const.GOTO_SEP < Const.DIAGRAM_PAD) this.goto_seperation_left += Const.GOTO_SEP;
			return Const.DIAGRAM_PAD - this.goto_seperation_left;
		}
		else {
			if (this.goto_seperation_right + Const.GOTO_SEP < Const.DIAGRAM_PAD) this.goto_seperation_right += Const.GOTO_SEP;
	...
	public void paintEntity(Graphics g) {
			...
			if (m.matches()) {
				...
				if (m.group(4) != null) {
					...
					/* END */
						...
						if (m.group(7).equals("AEnd")) e = new AEnd(graphics, id);
						else e = new End(graphics, id);
					}
					/* LINESPACER */
					...
					/* IF/FORK */
						...
						// if none is openend
						if (m.group(9).equals("Fork")) start_element = new Fork(graphics, id);
						else start_element = new If(graphics, id);
					...
					/* ENDIF/SYNC */
						...
						StopElement se;
						if (m.group(10).equals("Sync")) se = new Sync(graphics, id);
						else se = new EndIf(graphics, id);
					...
					/* GET EVENT */
					else if (m.group(15) != null) e = new EventRecieve(graphics, m.group(15), id);
					/* RAISE EVENT */
					else if (m.group(17) != null) e = new EventRaise(graphics, m.group(17), id);
					/* PARTACTIVITY */
					else if (m.group(19) != null) e = new PartActivity(m.group(19), graphics, id);
					/* ACTIVITY */
					else if (m.group(20) != null) e = new Activity(m.group(20), graphics, id);
				}
		...
		if (this.title != null) {
			...
			if ((title != null) && (title.length() > 0)) {
				this.getHandler().writeText(this.graphics, title, (int) (10 * zoom), (int) this.getHandler().getZoomedFontsize() + (int) this.getHandler().getZoomedDistLineToText(), false);
				int titlewidth = this.getHandler().getZoomedTextWidth(this.graphics, title);
				int ty = (int) this.getHandler().getZoomedFontsize() + (int) this.getHandler().getZoomedDistTextToLine() + (int) (8 * zoom);
		...
		if (width < Const.MIN_WIDTH) width = Const.MIN_WIDTH;
		if (height < Const.MIN_HEIGHT) height = Const.MIN_HEIGHT;
