...
public class DiagramFileHandler {
	...
	public boolean doSaveAs(String fileextension) {
		...
		if (extension.equals("uxf")) {
			...
			if (extension.equals("pdf")) this.doSaveAsPdf(fileName);
			else if (extension.equals("svg")) this.doSaveAsSvg(fileName);
			else if (extension.equals("eps")) this.doSaveAsEps(fileName);
			else this.doSaveAsIMG(extension, fileName);
	...
	public void doSaveAsSvg(String fileName) {
		...
		GenSvg.createAndOutputSvgToFile(fileName, this.handler);
		CustomElementSecurityManager.remThreadPrivileges(Thread.currentThread());
	}

	public void doSaveAsEps(String fileName) {
		CustomElementSecurityManager.addThreadPrivileges(Thread.currentThread(), fileName);
		GenEps.getInstance().createAndOutputEPSToFile(fileName, this.handler);
		CustomElementSecurityManager.remThreadPrivileges(Thread.currentThread());
	}

	public void doSaveAsPdf(String fileName) {
		CustomElementSecurityManager.addThreadPrivileges(Thread.currentThread(), fileName);
		GenPdf.getInstance().createAndOutputPdfToFile(fileName, this.handler);
		CustomElementSecurityManager.remThreadPrivileges(Thread.currentThread());
	}

	public void doSaveAsIMG(String imgtype, String fileName) {
		CustomElementSecurityManager.addThreadPrivileges(Thread.currentThread(), fileName);
		GenPic.getInstance().createAndOutputImgToFile(imgtype, fileName, this.handler);
		CustomElementSecurityManager.remThreadPrivileges(Thread.currentThread());
	}
