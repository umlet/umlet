...
public class DiagramHandler {
	...
	protected DiagramHandler(File diagram, boolean nolistener) {
		...
		if (this.helptext == null) this.helptext = Constants.defaultHelpText;
	...
	public Font getZoomedFont() {
		return new Font("SansSerif", Font.PLAIN, (int) getZoomedFontsize());
	}

	public Font getRealFont() {
		return new Font("SansSerif", Font.PLAIN, (int) getRealFontsize());
	}
	...
	public void setGridAndZoom(int factor, boolean zoomToTheCenter) {
		...
		if (zoomToTheCenter) {
			...
			diffx = ensureValueIsOnGrid(diffx);
			diffy = ensureValueIsOnGrid(diffy);
		...
		// We must realign everything to the grid, adjust the coordinates for the scrollbars and revalidate the drawpanel
		this.getDrawPanel().adjustLeftAndUpperCoordinates();
		// this.getDrawPanel().realignToGrid();
		this.getDrawPanel().revalidate();
