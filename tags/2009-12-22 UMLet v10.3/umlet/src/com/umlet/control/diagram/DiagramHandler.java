...
public class DiagramHandler {
	...
	protected DiagramHandler(File diagram, boolean nolistener) {
		...
		drawpanel.setStartUpHelpLabel(new StartUpHelpText(drawpanel)); // Create a new startup help panel
	...
	public void doSaveAs(String extension) {
		if (this.drawpanel.getAllEntities().isEmpty()) Umlet.displayError("You cannot save or export an empty diagram.");
		else if (this.fileHandler.doSaveAs(extension)) {
			this.setChanged(false);
			this.reloadPalettes();
		}
	}
	...
	public void setGridAndZoom(int factor, boolean zoomToTheCenter) {
		...
		for (Entity entity : getDrawPanel().getAllEntities()) {
			...
			entity.setLocation(realignToGrid(newX), realignToGrid(newY));
			...
			entity.setSize(realignToGrid(false, newW), realignToGrid(false, newH));
			...
			if (entity instanceof Relation) {
				for (Point point : ((Relation) entity).getLinePoints()) {
					...
					point.setLocation(realignToGrid(newX), realignToGrid(newY));
				}
			}
		}
		...
		if (zoomToTheCenter) {
			...
			for (Entity e : getDrawPanel().getAllEntities()) {
				// Groups are handled separately later
				if (!(e instanceof Group)) e.changeLocation(realignToGrid(false, diffx), realignToGrid(false, diffy));
			}
		}
