		...
		if (IS_DRAGGING == true) {
			...
			if ((RESIZE_DIRECTION & Constants.RESIZE_LEFT) > 0)
				if ((e.getWidth() - diffx) < this.handler.getFontsize())
					return;
				else if (RESIZE_DIRECTION == Constants.RESIZE_LEFT)
					diffy = 0;
			if ((RESIZE_DIRECTION & Constants.RESIZE_RIGHT) > 0) {
				if ((e.getWidth() + diffx) < this.handler.getFontsize())
					return;
				else if (RESIZE_DIRECTION == Constants.RESIZE_RIGHT)
					diffy = 0;
				diffw = diffx;
				diffx = 0;
			}
			...
			if ((RESIZE_DIRECTION & Constants.RESIZE_TOP) > 0)
				if ((e.getHeight() - diffy) < this.handler.getFontsize())
					return;
				else if (RESIZE_DIRECTION == Constants.RESIZE_TOP)
					diffx = 0;
			if ((RESIZE_DIRECTION & Constants.RESIZE_BOTTOM) > 0) {
				if ((e.getHeight() + diffy) < this.handler.getFontsize())
					return;
				else if (RESIZE_DIRECTION == Constants.RESIZE_BOTTOM)
					diffx = 0;
				diffh = diffy;
				diffy = 0;
			}
