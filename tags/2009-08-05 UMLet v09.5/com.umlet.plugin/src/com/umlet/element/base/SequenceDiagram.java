...
public class SequenceDiagram extends com.umlet.element.base.Entity {
	...
	public void assignToDiagram(DiagramHandler handler) {
		...
		rectHeight= 2*this.handler.getDistLineToText() + this.handler.getFontsize() + 10;
	}
	...
	public void paintEntity(Graphics g) {
    	...
    	if(lines.elementAt(0).startsWith("title:")) {
    		...
    		if(title != null && title.length()>0) {
	    		this.handler.writeText(g2,title,10,this.handler.getFontsize()+this.handler.getDistLineToText(),false);
	    		int titlewidth=this.handler.getPixelWidth(g2,title);
		...
		for (int i=0; i < numObjects; i++) {
		    ...
		    TextLayout layout= new TextLayout(s, this.handler.getFont(), g2.getFontRenderContext());
		    maxWidth= Math.max(layout.getBounds().getWidth(), maxWidth);
		}

		rectWidth= (int)Math.floor(maxWidth+1)+ 2* this.handler.getDistLineToText() + this.handler.getFontsize();
		...
		for (int i=0; i < numObjects; i++) {
		    ...
		    TextLayout layout= new TextLayout(s, this.handler.getFont(), 
		    ...
		    if (underline)
				g2.drawLine(tx, 
					    ty+this.handler.getDistTextToLine()/2, 
		...
		maxTextXpos+=3*this.handler.getDistTextToLine(); //add extra space
    ...
    private int printMethodName(Graphics2D g2, String methodName, 
    	...
    	Font font= this.handler.getFont();
    ...
    private void drawInteractionFrame(Graphics2D g2, int pos1, int pos2, int recursionLevel, String text) {
		...
		int x=this.handler.getDistTextToText()*2+recursionLevel*4;
		g2.drawRect(x,pos11,this.getWidth()-this.handler.getDistTextToText()*4-1-recursionLevel*8,h);
		int uLinePos = pos11+this.handler.getDistLineToText()+this.handler.getFontsize()+this.handler.getDistTextToLine();
		int textPos = pos11+this.handler.getDistLineToText()+this.handler.getFontsize();
		
		int textWidth = 0;
		if(text==null||text.equals("")) text=" ";
		g2.drawString(text,x+10,textPos);
		int pW = this.handler.getPixelWidth(g2,text);
