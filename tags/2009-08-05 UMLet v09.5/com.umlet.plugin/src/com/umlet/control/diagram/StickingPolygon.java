...
public class StickingPolygon {
	...
	public class StickLine
	{
		...
		public Point diffToLine(StickLine s, int x, int y)
		{
			...
			if(p2.x == p1.x)
			{
				diff.x = p1.x - (s.p2.x - s.p1.x) - x; //mitte der neuen linie
				if(s.p2.x == s.p1.x) {
				...
				diff.x = (x - p1.x) * (s.p2.x - s.p1.x) / (p2.x - p1.x) + s.p1.x - x;

			if(p2.y == p1.y)
			{
				diff.y = p1.y - (s.p2.y - s.p1.y) - y; //mitte der neuen linie
				if(s.p2.y == s.p1.y) {
				...
				diff.y = (x - p1.y) * (s.p2.y - s.p1.y) / (p2.y - p1.y) + s.p1.y - y;
		...
		private boolean isConnected(Point p)
		{
		    ...
		    double d=line.ptLineDist(p); //calculate distance 
		  	...
		  	return (d<5)&&((poly1.contains(p)||(poly2.contains(p)))); 
	...
	public void addPoint(Point p, boolean connect_to_first) {
		this.addPoint(p);
		if(connect_to_first && this.firstpoint != null)
	...
	private int isConnected(Point p)
	{
		...
		for(int i=0; i < this.stick.size(); i++)
			if(this.stick.get(i).isConnected(p))
				return i;
	...
	public Vector<RelationLinePoint> getStickingRelationLinePoints(DrawPanel panel)
	{
		...
		List<Relation> rels = panel.getAllRelationsOnPanel();
		for(Relation r : rels)
		{
			if(!r.isPartOfGroup())
			{
