...
public class Umlet {
	...
	private static Logger log = Logger.getLogger(Umlet.class);
	...
	private static void printUsage() {
		...
		for (String format : ImageIO.getWriterFileSuffixes())
			formats += "|" + format;
		printToConsole("USAGE: -action=convert -format=(" + formats + ") -filename=inputfile.uxf [-output=outputfile[.extension]]");
	}
	...
	private static boolean alreadyRunningChecker(boolean force) {
		try {
			File f = new File(Umlet.getInstance().getTempPath() + umlet_file);
			if (f.exists() && !force) return true;
			f.createNewFile();
			file_created = true;
			timer = new Timer(true);
			timer.schedule(new UmletRunningFileChecker(Umlet.getInstance().getTempPath() + umlet_file, Umlet.getInstance().getTempPath() + umlet_read_file), 0, 1000);
		} catch (Exception ex) {
			ex.printStackTrace();
			return true;
		}
	...
	private static boolean sendFileNameToRunningApplication(String filename) {
		...
		File f1 = new File(Umlet.getInstance().getTempPath() + umlet_file);
		...
		File f2 = new File(Umlet.getInstance().getTempPath() + umlet_read_file);
	...
	public static void main(String args[]) {
		...
		if (!(temporaryDirectory.endsWith("/")) && !(temporaryDirectory.endsWith("\\"))) temporaryDirectory = temporaryDirectory + System.getProperty("file.separator");
		umlet.setTempPath(temporaryDirectory);
	...
	private DiagramHandler currentinfodiagram;
	...
	public void setTempPath(String temppath) {
		Umlet.temp_directory = temppath;
	}

	public String getTempPath() {
		return Umlet.temp_directory;
	}
	...
	public void initLoggerConfiguration() {
		// Only if there is a log4j.properties file we activate logging
		if (new File(getHomePath() + "log4j.properties").exists()) {
			try {
				Properties props = new Properties();
				props.put("PROJECT_PATH", getHomePath()); // Put homepath as relative variable in properties file
				props.load(new FileInputStream(getHomePath() + "log4j.properties"));
	...
	public void setCurrentDiagram(DiagramHandler handler) {
		this.setCurrentInfoDiagram(handler);
		this.currentdiagram = handler;
		if (gui != null) gui.diagramSelected(handler);
	}

	public DiagramHandler getCurrentInfoDiagram() {
		return this.currentinfodiagram;
	}

	public void setCurrentInfoDiagram(DiagramHandler handler) {
		log.debug("trying to setCurrentInfoDiagram");
		log.debug("this.currentdiagram::");
		log.debug(this.currentinfodiagram);
		log.debug("handler::");
		log.debug(handler);
		if ((!(handler instanceof PaletteHandler)) && (handler != null)) {
			log.debug("SETTING currentInfoDiagram");
			this.currentinfodiagram = handler;
		}
	}
