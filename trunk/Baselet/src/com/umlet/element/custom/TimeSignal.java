...
public class TimeSignal extends OldGridElement {
	...
	public void paintEntity(Graphics g) {
		...
		int x0, y0, b, h;
		x0 = (int) (this.getDimension().width / 2 - 20 * zoom);
		y0 = 0;
		b = (int) (this.getDimension().width / 2 + 20 * zoom);
		h = (int) (40 * zoom);
		...
		for (int i = 0; i < tmp.size(); i++) {
			...
			if (s.equals("--")) {
				...
				g2.drawLine(this.getDimension().width / 2 - (int) this.getHandler().getFontHandler().getFontSize() * 4, yPos, this.getDimension().width / 2 + (int) this.getHandler().getFontHandler().getFontSize() * 4, yPos);
				yPos += (int) this.getHandler().getFontHandler().getDistanceBetweenTexts();
			}
			else {
