...
public abstract class Entity extends JComponent {
	...
	public void changeLocation(int diffx, int diffy) {
		...
		if (!this.isPartOfGroup()) this.setLocation(this.getX() + diffx, this.getY() + diffy);
	}

	protected void changeLocation(int diffx, int diffy, boolean fromgroup) {
		if (fromgroup) this.setLocation(this.getX() + diffx, this.getY() + diffy);
	}
	...
	public final void drawStickingPolygon(Graphics2D g2) { // LME: draw the sticking polygon
		...
		// OpenJRE starts at (1,1) with drawing while the official SunJRE starts at (0,0)
		if (System.getProperty("java.runtime.name").toUpperCase().contains("OPEN")) poly = this.generateStickingBorder(1, 1, this.getWidth() - 1, this.getHeight() - 1);
