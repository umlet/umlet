	     ...
	     for (Entity other : entities) {
	    	if(other instanceof Relation) { // a relation is always on top
	    		...
	    		if(other.contains(other_p))
	     		...
	     		if(other_bounds.contains(p))
	     		{	//the selected element has to be in front except if the other element is totally contained!
					if (bounds.contains(other_bounds) || (other.isSelected() && !other_bounds.contains(bounds)))
						return false;
	     		}
	     	}
	     }
...
public DiagramHandler getHandler() {
  ...
  public final void drawStickingPolygon(Graphics2D g2) { //LME: draw the sticking polygon
	StickingPolygon poly = this.generateStickingBorder(0, 0, this.getWidth()-1, this.getHeight()-1);
  	if(poly != null) {
	  	Color c = g2.getColor();
