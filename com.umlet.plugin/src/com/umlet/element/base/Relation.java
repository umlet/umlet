...
//					 Wirrer G. Start
				...
				String checkedString = Constants.checkStringForValidFormatsAndRemoveTheirLabels(str);
				...
				while (position != -1) {
					...
					if (position != -1) {
						...
						if (underline) {
							s = Constants.underlineLabel + s + Constants.underlineLabel;
						}
						if (bold) {
							s = Constants.boldLabel + s + Constants.boldLabel;
						}
						if (italic) {
							s = Constants.italicLabel + s + Constants.italicLabel;
						}
	...
	@Override
	public StickingPolygon generateStickingBorder(int x, int y, int width,
			int height) {
		return null;
	}
