...
public class Relation extends Entity {
	...
	public boolean lineUp(Vector<Rectangle> shapes, Vector<Point> points, int hotspotx, int hotspoty) {
		...
		for (int i = 0; i < shapes.size() - 1; i++) {
			...
			if (rMovable instanceof Arrow) {
				...
				a1 = Constants.normalize(a1, (int) this.getHandler().getZoomedFontsize());
				a2 = Constants.normalize(a2, (int) this.getHandler().getZoomedFontsize());
				...
				if (arrow.getString().equals("n")) {
					...
					newA1 = Constants.normalize(newA1, (int) this.getHandler().getZoomedFontsize() * 2);
					newA2 = Constants.normalize(newA2, (int) this.getHandler().getZoomedFontsize() * 2);
	...
	/*
	 * (non-Javadoc)
	 * @see com.umlet.element.base.Entity#paintEntity(java.awt.Graphics)
	 */
	/*
	 * (non-Javadoc)
	 * @see com.umlet.element.base.Entity#paintEntity(java.awt.Graphics)
	...
	public void paintEntity(Graphics g) {
		...
		this.getHandler().getFRC(g2); // Just to set anti-aliasing, even if no text
		// operations occur
		...
		if ((beginQualifier != null) && (beginQualifier.length() > 0)) {
			TextLayout tl = new TextLayout(beginQualifier, this.getHandler().getZoomedFont(), this.getHandler().getFRC(g2));
			Qualifier q = new Qualifier(beginQualifier, 0, 0, (int) tl.getBounds().getWidth()
					+ (int) this.getHandler().getZoomedFontsize() * 2, (int) tl.getBounds().getHeight()
					+ (int) this.getHandler().getZoomedFontsize() / 2);
			startShapes.add(q);
		}
		if ((endQualifier != null) && (endQualifier.length() > 0)) {
			TextLayout tl = new TextLayout(endQualifier, this.getHandler().getZoomedFont(),
		...
		if ((beginMultiplicity != null) && (beginMultiplicity.length() > 0)) {
			EmptyShape e = new EmptyShape((int) this.getHandler().getZoomedFontsize());
			startShapes.add(e);
			TextLayout tl = new TextLayout(beginMultiplicity, this.getHandler().getZoomedFont(), this.getHandler().getFRC(g2));
		...
		if ((endMultiplicity != null) && (endMultiplicity.length() > 0)) {
			EmptyShape e = new EmptyShape((int) this.getHandler().getZoomedFontsize());
			endShapes.add(e);
			TextLayout tl = new TextLayout(endMultiplicity, this.getHandler().getZoomedFont(), this.getHandler().getFRC(g2));
		...
		if ((beginRole != null) && (beginRole.length() > 0)) {
			...
			int position = 0;
			int lineBreaks = 0;
			double broadestText = this.getHandler().getZoomedTextWidth(g2, beginRole);
			while (position != 1) {
				...
				if (positionNew != -1) {
					...
					broadestText = Math.max(broadestText, this.getHandler()
							.getZoomedTextWidth(g2, beginRole.substring(position,
							positionNew)));
					if (beginRole.lastIndexOf("\\\\") + 2 != beginRole.length()) broadestText = Math.max(broadestText, this.getHandler().getZoomedTextWidth(g2,
							beginRole.substring(beginRole.lastIndexOf("\\\\") + 2, beginRole.length())));
					lineBreaks++;
				}
			...
			Role r = new Role(beginRole, 0, 0, (int) broadestText, lineBreaks
					* (int) this.getHandler().getZoomedFontsize() + (lineBreaks + 2)
					* (int) this.getHandler().getZoomedDistTextToText());

			// <OLDCODE>
			/*
			 * TextLayout tl = new TextLayout(beginRole, this.getHandler().getFont(),
			 * this.getHandler().getFRC(g2)); Role r = new Role(beginRole, 0, 0, (int)
		...
		if ((endRole != null) && (endRole.length() > 0)) {
			...
			int position = 0;
			int lineBreaks = 0;
			double broadestText = this.getHandler().getZoomedTextWidth(g2, endRole);
			while (position != 1) {
				...
				if (positionNew != -1) {
					...
					broadestText = Math.max(broadestText, this.getHandler().getZoomedTextWidth(g2,
							endRole.substring(position, positionNew)));
					if (endRole.lastIndexOf("\\\\") + 2 != endRole.length()) broadestText = Math.max(broadestText,
							this.getHandler().getZoomedTextWidth(g2, endRole.substring(endRole.lastIndexOf("\\\\") + 2,
							endRole.length())));
					lineBreaks++;
				}
			 ...
			 * this.getHandler().getFRC(g2)); Role r = new Role(endRole, 0, 0, (int)
			 * tl.getBounds().getWidth(), (int) tl.getBounds().getHeight());
			 */
			// </OLDCODE>
			// A.Mueller end
		...
		if ((beginPort != null) && (beginPort.length() > 0)) {
			...
			EmptyShape e = new EmptyShape((int) this.getHandler().getZoomedFontsize());
			startShapes.add(e);
			TextLayout tl = new TextLayout(beginPort, this.getHandler().getZoomedFont(), this.getHandler().getFRC(g2));
		...
		if ((endPort != null) && (endPort.length() > 0)) {
			EmptyShape e = new EmptyShape((int) this.getHandler().getZoomedFontsize());
			endShapes.add(e);
			TextLayout tl = new TextLayout(endPort, this.getHandler().getZoomedFont(), this.getHandler().getFRC(g2));
		...
		else if (lineType.equals(".-")) g2.setStroke(Constants.getStroke(1, 1));
		else if (lineType.equals("-")) g2.setStroke(Constants.getStroke(0, 1));
		else if (lineType.equals(".")) g2.setStroke(Constants.getStroke(1, 1));
		...
		for (int i = 0; i < tmp.size(); i++) {
			...
			if (r instanceof Qualifier) {
				...
				this.getHandler().writeText(g2, q.getString(), (int) r.getX()
						+ (int) this.getHandler().getZoomedFontsize(), (int) r.getY()
						+ (int) this.getHandler().getZoomedFontsize(), false);
				...
				// A.Mueller Begin
					...
					if (beginArrow.startsWith("compStart")) {
						...
						if (!csdStartText.equals("")) s = this.getHandler().getZoomedTextWidth(g2, csdStartText);
						...
						if (csdStartText.equals(">")) {
							...
							if (!csdStartText.equals("")) {
								this.getHandler().writeText(g2, csdStartText, px1.x + (int) (3 * zoom), px1.y + (int) (6 * zoom), true);
							}
						}
						...
						if (beginArrow.equals("compStartdel")) {
							...
							at2.rotate(winkel, cx, cy);
							g2.setTransform(at2);
							g2.drawLine((int) (px1.x + s / 2 + 2 * zoom), px1.y, (int) (px1.x + s / 2 + arrowThreeLength), px1.y - arrowThreeSize);
							g2.drawLine((int) (px1.x + s / 2 + 2 * zoom), px1.y, (int) (px1.x + s / 2 + arrowThreeLength), px1.y + arrowThreeSize);
							g2.setTransform(at);
						}
					...
					if (endArrow.startsWith("compEnd")) {
						...
						if (!csdEndText.equals("")) s = this.getHandler().getZoomedTextWidth(g2, csdEndText);
						if (s < boxSize) s = boxSize;
						...
						if (csdEndText.equals(">")) {
							...
							if (!csdEndText.equals("")) {
								this.getHandler().writeText(g2, csdEndText, px1.x + (int) (3 * zoom), px1.y + (int) (6 * zoom), true);
							}
						}
					...
					g2.setFont(this.getHandler().getZoomedFont()); // reset font
				...
				// (int)r.getHeight());
				this.getHandler().writeText(g2, m.getString(), (int) r.getX(), (int) r.getY()
						+ (int) this.getHandler().getZoomedFontsize() + 2 * (int) this.getHandler().getZoomedDistTextToText(), false); // B. Buckl
				// added
				...
				String checkedString = DiagramHandler.checkStringForValidFormatsAndRemoveTheirLabels(str);
				...
				int position = 0;
				int y = 4 * (int) this.getHandler().getZoomedDistLineToText();
				while (position != -1) {
					position = str.indexOf("\\\\");

					if (position != -1) {
						String s = str.substring(0, position);
						if (underline) s = Constants.FormatLabels.UNDERLINE + s + Constants.FormatLabels.UNDERLINE;
						if (bold) s = Constants.FormatLabels.BOLD + s + Constants.FormatLabels.BOLD;
						if (italic) s = Constants.FormatLabels.ITALIC + s + Constants.FormatLabels.ITALIC;
						this.getHandler().writeText(g2, s, (int) r.getX(), (int) r.getY() + y, false);

						y = y + (int) this.getHandler().getZoomedFontsize();
						str = str.substring(position + 2, str.length());
					}
					else {
						if (bold) str = Constants.FormatLabels.BOLD + str + Constants.FormatLabels.BOLD;
						if (italic) str = Constants.FormatLabels.ITALIC + str + Constants.FormatLabels.ITALIC;
						if (underline) str = Constants.FormatLabels.UNDERLINE + str + Constants.FormatLabels.UNDERLINE;
						this.getHandler().writeText(g2, str, (int) r.getX(), (int) r.getY() + y, false);
				 ...
				 * .getY() + this.getHandler().getFontsize() + 2
				...
				this.getHandler().writeText(g2, p.getString(),
						(int) (r.getX()),
						(int) (r.getY()), false);
		...
		if (this.getStrings() != null) {
			if (this.getStrings().size() > 0) {
				...
				int yPos = start.y - (int) this.getHandler().getZoomedDistTextToText(); // B. Buckl
				// added
				...
				for (int i = 0; i < getStrings().size(); i++) {
					...
					if (s.startsWith(">") || s.endsWith(">") || s.startsWith("<") || s.endsWith("<")) {
						...
						if (((s.indexOf(">") == s.lastIndexOf(">")) && (s.indexOf(">") != -1))
							...
							int fontHeight = g2.getFontMetrics(
									this.getHandler().getZoomedFont()).getHeight()
									- g2.getFontMetrics(this.getHandler().getZoomedFont()).getDescent()
									- g2.getFontMetrics(this.getHandler().getZoomedFont()).getLeading();
							fontHeight = fontHeight / 3 * 2;

							if (s.endsWith(">")) {
								s = s.substring(0, s.length() - 1);
								int fontWidth = this.getHandler().getZoomedTextWidth(g2, s);
					...
					this.getHandler().writeText(g2, s, xPos, yPos, true);
		...
		if (this.getStrings() != null) {
			if (this.getStrings().size() > 0) {
				...
				for (int i = 0; i < getStrings().size(); i++) {
					String s = this.getStrings().elementAt(i);
					int width = this.getHandler().getZoomedTextWidth(g2, s);
					criticalPoints.add(new Point(xPos - width / 2 - (int) (20 * zoom), yPos - (int) this.getHandler().getZoomedFontsize() - (int) (20 * zoom)));
	...
	public void reLocate() { // LME
		...
		for (int vi = 0; vi < ve.size(); vi++) {
			Point p = ve.elementAt(vi);
			int gridUnit = this.getHandler().getGridSize();
			int dux = (int) (this.getX() + p.getX());
			int duy = (int) (this.getY() + p.getY());
