...
public class Relation extends Entity {
	...
	public void paintEntity(Graphics g) {
			...
			if (r instanceof Qualifier) {
				...
				while (position != -1) {
					...
					if (position != -1) {
						...
						if (underline) s = Constants.underlineLabel + s + Constants.underlineLabel;
						if (bold) s = Constants.boldLabel + s + Constants.boldLabel;
						if (italic) s = Constants.italicLabel + s + Constants.italicLabel;
						...
						if (bold) str = Constants.boldLabel + str + Constants.boldLabel;
						if (italic) str = Constants.italicLabel + str + Constants.italicLabel;
						if (underline) str = Constants.underlineLabel + str + Constants.underlineLabel;
		...
		if (this.getStrings() != null) {
			if (this.getStrings().size() > 0) {
				...
				for (int i = 0; i < getStrings().size(); i++) {
					...
					criticalPoints.add(new Point(xPos - width / 2 - 20, yPos - (int) this.getHandler().getZoomedFontsize() - 20));
		...
		for (int i = 0; i < getLinePoints().size(); i++) {
			...
			// Subtract and add 20 to avoid cutting the circles at the end of the relation
