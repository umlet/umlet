...
public class Fork extends StartElement {
	...
	public void paint() {
		...
		this.getGraphics().fillRect(x - w / 2, y - (h + pad) / 2, w, (int) (4 * zoom));
	}
	...
	protected Point getNonStdConnectIn(Direction dir) {
		...
		if (this.arrowIn()) Connector.drawArrow(this.getGraphics(), c.x, c.y - (int) (10 * zoom), c.x, c.y);
		else this.getGraphics().drawLine(c.x, c.y - (int) (10 * zoom), c.x, c.y);
		c.y -= (int) (10 * zoom);
		return c;
	}
	...
	protected Point getNonStdConnectOut(Direction dir) {
		...
		if (dir.equals(Direction.LEFT)) {
			this.getGraphics().drawLine(c.x - (int) (10 * zoom), c.y + (int) (3 * zoom), c.x, c.y);
			c.x -= (int) (10 * zoom);
			c.y += (int) (3 * zoom);
		}
	...
	public void connectTo(Element e) {
		if (e != null) {
			if (this.connectOut_overrideable() && e.connectIn()) {
				...
				if (this.arrowOut() && e.arrowIn()) Connector.drawArrow(this.getGraphics(), from.x, from.y, to.x, to.y);
