...
public class Connector {
	...
	public static void drawArrow(Graphics2D graphics, int x1, int y1, int x2, int y2) {
		...
		ax1=Math.sqrt(Math.pow(x2-x1, 2) + Math.pow(y2-y1,2)) - 7;
		ax2=ax1;
		ay1=-4;
		ay2=5;		
	...
	public void paint() {
		if(e1.connectOut() && e2.connectIn()) {
			Coordinate c2 = this.e2.getConnect(Direction.TOP);
			Coordinate c1 = this.e1.getConnect(Direction.BOTTOM);
			
			if(c1 != null && c2 != null) {
				if(!c1.equals(c2)) {
					if(e1.arrowOut() && e2.arrowIn())
						Connector.drawArrow(this.graphics, c1.x, c1.y, c2.x, c2.y);
					else
						this.graphics.drawLine(c1.x, c1.y, c2.x, c2.y);
				}
			}
		}
	}
