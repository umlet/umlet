...
public class While extends Container {
	...
	public While(Graphics2D g, Container parent, ArrayList<Row> rows, int row, String condition) {
		...
		if ((condition != null) && !condition.equals("")) this.while_element = new Condition(condition, g);
		else this.while_element = new LineSpacer(g);
	...
	protected int getLeftWidth() {
		...
		if (this.getColumns().size() > 1) width += this.getColumns().get(1).getLeftWidth() + Const.COLUMN_PAD;
		return width;
	}
