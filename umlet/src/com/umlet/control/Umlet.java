...
public class Umlet {
	...
	private static Logger log = Logger.getLogger(Umlet.class.getName());
	...
	private static void printUsage() {
		...
		for (String format : ImageIO.getWriterFileSuffixes())
			formats += "|" + format;
		System.out.println("USAGE: -action=convert -format=(" + formats + ") -filename=inputfile.uxf [-output=outputfile[.extension]]");
	}

	public static void doConvert(String fileName, String format, String outputfilename) {
		File file = new File(fileName);
		if (!file.exists()) {
			printError("File '" + file.getAbsolutePath() + "' not found.");
			return;
		}
	...
	private static boolean alreadyRunningChecker(boolean force) {
		try {
			File f = new File(Umlet.getInstance().getHomePath() + umlet_file);
	...
	private static boolean sendFileNameToRunningApplication(String filename) {
		...
		File f1 = new File(Umlet.getInstance().getHomePath() + umlet_file);
		...
		File f2 = new File(Umlet.getInstance().getHomePath() + umlet_read_file);
	...
	public void close() {
		...
		if (file_created) {
			timer.cancel();
			(new File(Umlet.getInstance().getHomePath() + umlet_file)).delete();
		}
	}
}
