...
public class UMLetContributor extends EditorActionBarContributor {
	...
	private static Logger log = Logger.getLogger(UMLetContributor.class);

	private static boolean diagramActionLocked = false;
	private static final int DELAY = 100;

	private class DelayTimerTask extends TimerTask {
		@Override
		public void run() {
			diagramActionLocked = false;
		}
	}
	...
	private ArrayList<ZoomAction> zoomactions;
	...
	public UMLetContributor() {
		...
		this.zoomactions = new ArrayList<ZoomAction>();
	...
	public void init(IActionBars actionBars) {
		...
		copyActionDiagram = new Action() {
			@Override
			public void run() {
				// After the call all diagram actions get locked for a short time period to avoid a bug with zoom to 100% before the command
				if (!diagramActionLocked) {
					diagramActionLocked = true;
					DiagramHandler handler = Umlet.getInstance().getDiagramHandler();
					if (handler != null) handler.getController().executeCommand(new Copy());
					new Timer().schedule(new DelayTimerTask(), DELAY);
				}
			}
		};
	...
	private void createMenuActions() {
		...
		rateUmlet = new Action("Rate UMLet at Eclipse Marketplace...") {
			@Override
			public void run() {
		...
		this.editPalette = new Action("Edit Current Palette") {
			@Override
			public void run() {
			...
			// String filePath = Umlet.getInstance().getPalette().getFileHandler().getFullPathName();
	...
	public void contributeToMenu(IMenuManager manager) {
		...
		for (ZoomAction a : this.zoomactions)
			zoom.add(a);
	...
	public void setZoomLevel(float zoomFactor) {
		zoomFactor = zoomFactor * 10;
		Integer factorAsInteger = (int) zoomFactor;
		String factorAsString = factorAsInteger.toString();
		for (ZoomAction a : this.zoomactions) {
			if (a.zoomFactor.equals(factorAsString)) a.setChecked(true);
