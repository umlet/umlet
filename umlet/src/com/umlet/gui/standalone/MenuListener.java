...
import com.umlet.control.command.Align;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ((e.getSource() instanceof JCheckBoxMenuItem) && (handler != null)) {
			...
			if (handler != null) // only execute if at least one diagram is available
			{
				...
				if (b.getActionCommand().startsWith("align_")) {
					Vector<Entity> v = selector.getSelectedEntities();
					if (v.size() > 0) {
						handler.getController().executeCommand(
								new Align(v, selector.getDominantEntity(), b.getActionCommand().substring("align_".length())));
					}
				}
				else if (b.getText() == "Delete") {
					Vector<Entity> v = selector.getSelectedEntities();
					if (v.size() > 0) {
						handler.getController().executeCommand(
								new RemoveElement(v));
