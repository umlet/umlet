...
public class EntityListener extends UniversalListener {
	...
	public void mouseMoved(MouseEvent me) {
		...
		if(ra == 0) Umlet.getInstance().getGUI().setCursor(Constants.HAND_CURSOR);
	...
	private void pressedLeftButton(MouseEvent me){
		...
		if ((me.getModifiers() & Constants.CTRLMETA_MASK) != 0) initializeLasso(me);
...
//		this.IS_FIRST_DRAGGING_OVER=true;
	...
	private void initializeLasso(MouseEvent me) {
		...
		Umlet.getInstance().getDiagramHandler().getDrawPanel().add(selframe, 0);
		Umlet.getInstance().getGUI().setCursor(Constants.DEFAULT_CURSOR);
	}

	private void dragLasso(MouseEvent me, Entity e) {
	...
	private void dragEntity(Entity e) {
		// log.debug("IS_DRAGGING");
	...
	private void firstDragging(){
		Point newp = this.getNewCoordinate();
		Point oldp = this.getOldCoordinate();
		int diffx = newp.x - oldp.x;
		int diffy = newp.y - oldp.y;		
		Vector<Entity> entitiesToBeMoved = this.selector.getSelectedEntities();
		...
		for (Entity eToBeMoved : entitiesToBeMoved) {
			...
			if (eToBeMoved instanceof Relation) continue;
			StickingPolygon stick = null;
			if (eToBeMoved.isStickBorders()) stick = eToBeMoved.generateStickingBorder(eToBeMoved.getX(), eToBeMoved.getY(), eToBeMoved.getWidth(), eToBeMoved.getHeight());
			if (stick != null) {
	...
	private void resizeEntity(Entity e) {
		int gridSize = Umlet.getInstance().getDiagramHandler().getGridSize();
		int delta_x = 0;
		int delta_y = 0;
		Point newp = this.getNewCoordinate();
