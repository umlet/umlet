...
public abstract class CustomElement extends Entity {
	...
	private class Text {
		...
		private Text(String text, int x, int y, int align, int valign) {
			this(text, x, y, align, valign, null);
		}

		private Text(String text, int x, int y, int align, int valign, Integer fixedSize) {
			this.text = text;
			this.x = x;
			this.y = y;
			this.align = align;
	...
	private void drawShapes() {
		...
		for (StyleShape s : this.shapes) {
			specialBgColor = ((s.getBgColor() != Constants.DEFAULT_BACKGROUND_COLOR) || (s.getAlpha() != Constants.DEFAULT_ALPHA));
			if (specialBgColor) {
		...
		for (StyleShape s : this.shapes) {
			specialLine = ((s.getLineType() != Constants.DEFAULT_LINE_TYPE) || (s.getLineThickness() != Constants.DEFAULT_LINE_THICKNESS));
			specialFgColor = (s.getFgColor() != Constants.DEFAULT_FOREGROUND_COLOR);

			if (specialLine) g2.setStroke(Constants.getStroke(s.getLineType(), s.getLineThickness()));
			if (specialFgColor) g2.setColor(s.getFgColor());
			this.g2.draw(s.getShape());
			if (specialLine) g2.setStroke(Constants.getStroke(Constants.DEFAULT_LINE_TYPE, Constants.DEFAULT_LINE_THICKNESS));
			if (specialFgColor) g2.setColor(activeColor);
		}

		for (Text t : this.texts) {
			if (t.fixedSize != null) {
				oldFontSize = getHandler().getFontsize();
				getHandler().setFontsize(t.fixedSize);
				g2.setFont(this.getHandler().getRealFont());
				t.y += textHeight();
			}
			this.getHandler().writeText(this.g2, t.text, t.x, t.y, t.align, t.valign);
			if (t.fixedSize != null) {
				getHandler().setFontsize(oldFontSize);
	...
	protected final void print(String text, int x, int y) {
		this.texts.add(new Text(text, (int) (x * zoom), (int) (y * zoom), Constants.LEFT, Constants.BOTTOM));
	}

	@CustomFunction(param_defaults = "text,y")
	protected final void printLeft(String text, int y) {
		this.texts.add(new Text(text, (int) this.getHandler().getZoomedDistLineToText(), (int) (y * zoom), Constants.LEFT, Constants.BOTTOM));
	}

	@CustomFunction(param_defaults = "text,y")
	protected final void printRight(String text, int y) {
		this.texts.add(new Text(text, (int) (width * zoom) - this.zoomedTextsize(text).width, (int) (y * zoom), Constants.LEFT, Constants.BOTTOM));
	}

	@CustomFunction(param_defaults = "text,y")
	protected final void printCenter(String text, int y) {
		this.texts.add(new Text(text, (onGrid((int) (width * zoom)) - this.zoomedTextsize(text).width) / 2, (int) (y * zoom), Constants.LEFT, Constants.BOTTOM));
	}
	...
	protected final void printFixedSize(String text, int x, int y, int fixedFontSize) {
		this.texts.add(new Text(text, (int) (x * zoom), (int) (y * zoom), Constants.LEFT, Constants.BOTTOM, fixedFontSize));
	}

	@CustomFunction(param_defaults = "")
	protected final int textHeight() {
		return (int) ((int) this.getHandler().getRealFontsize() + this.getHandler().getRealdDistTextToText());
	}
	...
	protected final void setForegroundColor(String fgColorString) {
		tmpFgColor = Constants.getColor(fgColorString);
		if (tmpFgColor == null) {
			if (fgColorString.equals("fg")) tmpFgColor = activeColor;
	...
	protected final void setBackgroundColor(String bgColorString) {
		tmpBgColor = Constants.getColor(bgColorString);
		if (tmpBgColor == null) {
			if (bgColorString.equals("bg")) tmpBgColor = fillColor;
			else tmpBgColor = Constants.DEFAULT_BACKGROUND_COLOR;
		}
		// Transparency is 0% if none or 50% if anything else
		if (bgColorString.equals("none")) tmpAlpha = Constants.TRANSPARENT_ALPHA;
		else tmpAlpha = Constants.DEFAULT_ALPHA;
	}

	@CustomFunction(param_defaults = "")
	protected final void resetAll() {
		tmpLineThickness = Constants.DEFAULT_LINE_THICKNESS;
		tmpLineType = Constants.DEFAULT_LINE_TYPE;
