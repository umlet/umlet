...
public class SequenceDiagram extends com.umlet.element.base.Entity {
	...
	public void paintEntity(Graphics g) {
		...
		if (lines.elementAt(0).startsWith("title:")) {
			...
			if ((title != null) && (title.length() > 0)) {
				this.getHandler().writeText(g2, title, (int) (5 * zoom), (int) this.getHandler().getZoomedFontsize() + (int) this.getHandler().getZoomedDistLineToText(), false);
				int titlewidth = this.getHandler().getZoomedTextWidth(g2, title);
				int ty = (int) (8 * zoom) + (int) (this.getHandler().getZoomedFontsize() + this.getHandler().getZoomedDistTextToLine());
		...
		obj = Constants.decomposeStrings(newhead.length() > 0 ? newhead.substring(1) : "", "|");
		...
		for (int i = 1; i < lines.size(); i++) {
			...
			for (int j = 0; j < interactions.size(); j++) {
				...
				else // LABLING ADD
				{
					...
					if (group.length() > 0) group = group.substring(1);
				} // STOP LABLING ADD
		...
		for (int i = 0; i < numObjects; i++) {
			...
			if (s.startsWith(Constants.FormatLabels.UNDERLINE) && s.endsWith(Constants.FormatLabels.UNDERLINE) && (s.length() > 2)) {
				s = s.substring(1, s.length() - 1);
			}
			TextLayout layout = new TextLayout(s, this.getHandler().getZoomedFont(), g2.getFontRenderContext());
			maxWidth = Math.max(layout.getBounds().getWidth(), maxWidth);
			maxHeight = Math.max(layout.getBounds().getHeight(), maxHeight);
		}

		rectWidth = (int) Math.floor(maxWidth + 1) + 2 * (int) this.getHandler().getZoomedDistLineToText() + (int) this.getHandler().getZoomedFontsize();
		rectHeight = (int) Math.floor(maxHeight + 1) + (int) this.getHandler().getZoomedDistLineToText() + (int) this.getHandler().getZoomedFontsize();
		...
		for (int i = 0; i < numObjects; i++) {
			...
			TextLayout layout = new TextLayout(s, this.getHandler().getZoomedFont(),
			...
			if (underline) g2.drawLine(tx,
					ty + (int) this.getHandler().getZoomedDistTextToLine() / 2,
		...
		maxTextXpos += 3 * this.getHandler().getZoomedDistTextToLine(); // add extra space
	...
	private int printMethodName(Graphics2D g2, String methodName,
		...
		Font font = this.getHandler().getZoomedFont();
	...
	private void drawInteractionFrame(Graphics2D g2, int pos1, int pos2, int recursionLevel, String text) {
		...
		int x = (int) this.getHandler().getZoomedDistTextToText() * 2 + recursionLevel * 4;
		g2.drawRect(x, pos11, this.getWidth() - (int) this.getHandler().getZoomedDistTextToText() * 4 - 1 - recursionLevel * 8, h);
		int uLinePos = pos11 + (int) this.getHandler().getZoomedDistLineToText() + (int) (this.getHandler().getZoomedFontsize() + (int) this.getHandler().getZoomedDistTextToLine());
		int textPos = pos11 + (int) this.getHandler().getZoomedDistLineToText() + (int) this.getHandler().getZoomedFontsize();

		int textWidth = 0;
		if ((text == null) || text.equals("")) text = " ";
		g2.drawString(text, x + (int) (10 * zoom), textPos);
		int pW = this.getHandler().getZoomedTextWidth(g2, text);
	...
	private void zoomValues() {
		...
		rectHeight = 2 * (int) this.getHandler().getZoomedDistLineToText() + (int) this.getHandler().getZoomedFontsize() + 10;
		rectWidth = 2 * (int) this.getHandler().getZoomedDistLineToText() + (int) this.getHandler().getZoomedFontsize() + 10;
