...
public class SynchBarVertical extends Entity {
	...
	public void paintEntity(Graphics g) {
		...
		textHeight = tmp.size() * ((int) (this.getHandler().getZoomedFontsize() + this.getHandler().getZoomedDistTextToText()));
		boolean ADAPT_SIZE = false;

		for (int i = 0; i < tmp.size(); i++) {
			String s = tmp.elementAt(i);
			yPos += (int) this.getHandler().getZoomedFontsize();
			TextLayout l = new TextLayout(s, this.getHandler().getZoomedFont(), Constants.getFRC(g2));
		...
		if (yPos > this.getHeight()) {
			(new Resize(this, 0, 0, 0, (int) (this.getHandler().getZoomedFontsize() + this.getHandler().getZoomedDistTextToText()))).execute(this.getHandler());
			return;
		}

		g2.fillRect(this.getWidth() / 2 - (int) (3 * zoom), textHeight + (int) this.getHandler().getZoomedDistTextToLine(), (int) (5 * zoom), this.getHeight() - textHeight - (int) this.getHandler().getZoomedDistTextToLine() * 2);
	}
