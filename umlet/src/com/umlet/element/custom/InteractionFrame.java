...
public class InteractionFrame extends Entity {
	...
	public void paintEntity(Graphics g) {
		...
		float yPos = 0;
		yPos += this.getHandler().getZoomedDistLineToText();
		...
		for (int i = 0; i < tmp.size(); i++) {
			...
			if (s.equals("--")) {
				...
				topHeight = i;
				yPos += (this.getHandler().getZoomedDistTextToLine() + this.getHandler().getZoomedDistLineToText());
				center = true;
			}
			else if (s.equals("-.")) {
				...
				yPos += this.getHandler().getZoomedFontsize();
				if (center) this.getHandler().writeText(g2, s, this.getWidth() / 2, (int) yPos, true);
				else this.getHandler().writeText(g2, s, (int) this.getHandler().getZoomedFontsize() / 2, (int) yPos,
						false);
