...
function activate(context) {
    ...
    const previewAndCloseSrcDoc = (document) => __awaiter(this, void 0, void 0, function* () {
        ...
        if (document.fileName.split('.').pop() === "uxf") {
            ...
            yield vscode.commands.executeCommand("extension.helloWorld", document.getText().toString());
        }
    });
    const openedEvent = vscode.workspace.onDidOpenTextDocument((document) => {
        previewAndCloseSrcDoc(document);
    });
    let disposable = vscode.commands.registerCommand('extension.helloWorld', (fileContents) => {
...
function GetCacheOneJavascript(extensionPath) {
    ...
    let positionInText = data === null || data === void 0 ? void 0 : data.indexOf(pathReplacementTag);
    console.log("found text in :" + positionInText);
    if (positionInText !== undefined && data !== undefined) {
        data = data === null || data === void 0 ? void 0 : data.replace(pathReplacementTag, extensionPath);
...
/**
  ...
  * @param localUmletFolder folder which holds the local umletino gwt version.
  */
function GetUmletWebviewPageNoPreload(localUmletFolder) {
    return `<!DOCTYPE html>
   <html>
     <head>
       <base href="${localUmletFolder}/" />
