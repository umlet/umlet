...
public class DrawHandlerGwt extends DrawHandler {
	...
	public void printHelper(final StringStyle[] text, final PointDouble point, final AlignHorizontal align) {
		...
		addDrawable(new DrawFunction() {
			...
			public void run() {
				...
				ColorOwnLight fgColor = getOverlay().getForegroundColor() != null ? getOverlay().getForegroundColor() : styleAtDrawingCall.getForegroundColor();
	...
	private void setStyle(Context2d ctx, Style style) {
		...
		ColorOwnLight fgColor = getOverlay().getForegroundColor() != null ? getOverlay().getForegroundColor() : style.getForegroundColor();
		if (fgColor != null) {
