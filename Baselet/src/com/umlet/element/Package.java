...
public class Package extends OldGridElement {
	...
	public void paintEntity(Graphics g) {
		...
		int maxUpperBox = (int) (this.getDimension().width * 0.4); // I think this looks better
		int lines = 0;
		...
		for (int i = 0; i < tmp.size(); i++) {
			...
			if (s.equals("--") && (borders == false)) {
				...
				g2.fillRect(0, yPosBorder, this.getDimension().width - 1, this.getDimension().height - yPosBorder - 1);
				g2.setComposite(composites[0]);
				if (isSelected) g2.setColor(fgColor);
				else g2.setColor(fgColorBase);

				g2.drawRect(0, 0, maxUpperBox, yPosBorder);
				g2.drawRect(0, yPosBorder, this.getDimension().width - 1, this.getDimension().height - yPosBorder - 1);
				// yPos to write the String centered
				yPosBorder = (int) this.getHandler().getFontHandler().getFontSize() / 2 + yPosBorder / 2 + this.getDimension().height / 2 - (tmp.size() - lines) * ((int) (this.getHandler().getFontHandler().getFontSize() + (int) this.getHandler().getFontHandler().getDistanceBetweenTexts())) / 2;
			}
		...
		for (int i = 0; i < tmp.size(); i++) {
			...
			if (s.equals("--") && (borders == false)) {
				...
				// yPos to write the String centered
				yPos = (int) this.getHandler().getFontHandler().getFontSize() / 2 + yPos / 2 + this.getDimension().height / 2 - (tmp.size() - lines) * ((int) (this.getHandler().getFontHandler().getFontSize() + this.getHandler().getFontHandler().getDistanceBetweenTexts())) / 2;
			}
			else if (!normal && (i > 1) && (tmp.elementAt(i - 1).equals("--") && tmp.elementAt(i).startsWith("left:"))) {
