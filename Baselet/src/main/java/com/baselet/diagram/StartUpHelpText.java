...
public class StartUpHelpText extends JEditorPane
    ...
    public StartUpHelpText(DrawPanel panel) {
        ...
        // If the GUI is null (e.g.: if main is used in batch mode) the startup
        // help text is not required
        ...
        try {
            if (UpdateCheckTimerTask.getInstance().getFilename() == null) {
                showHTML(createTempFileWithText(
                        getDefaultTextWithReplacedSystemspecificMetakeys()));
            } else {
    ...
    // Must be overwritten to hide the helptext if a the custom elements panel
    // is toggled without elements on the drawpanel
    @Override
    ...
    static String createTempFileWithText(String textToWriteIntoFile)
            throws IOException {
        File tempFile = File.createTempFile(
                Program.getInstance().getProgramName() + "_startupfile",
                ".html");
    ...
    private static String getDefaultTextWithReplacedSystemspecificMetakeys()
        ...
        try {
            ...
            while (sc.hasNextLine()) {
                ...
                if (SystemInfo.META_KEY == Metakey.CTRL) {
                    line = line.replace(Metakey.CMD.toString(),
                            Metakey.CTRL.toString());
                } else if (SystemInfo.META_KEY == Metakey.CMD) {
                    line = line.replace(Metakey.CTRL.toString(),
                            Metakey.CMD.toString());
                }
    ...
    public void componentAdded(ContainerEvent e) {
        boolean gridElementAdded = panel
                .getElementToComponent(e.getChild()) != null;
        if (gridElementAdded) {
            setVisible(false);
        }
    }
    ...
    public void componentResized(ComponentEvent arg0) {
        ...
        int labelSizeToSubtract = Math.max(150, labelSize.height); // the upper
                                                                   // border of
                                                                   // the
                                                                   // startup
                                                                   // panel is
                                                                   ...
                                                                   // info
                                                                   // windows)
        this.setLocation(size.width / 2 - labelSize.width / 2, Math.max(
                minDistanceFromTop, size.height / 2 - labelSizeToSubtract));
    }
    ...
    public void paint(Graphics g) {
        ...
        ((Graphics2D) g)
                .setRenderingHints(Utils.getUxRenderingQualityHigh(false));
        super.paint(g);
        ((Graphics2D) g)
                .setRenderingHints(Utils.getUxRenderingQualityHigh(true));
    }
    ...
    /**
     * The MouseListener of this JEditorPane just delegates the MouseEvents up
     * to the DiagramListener of the Handler
     */
